...
def main():
    ...
    initial_location = input("Enter location of vacuum: (A or B): ").upper()[0]
    ...
    cost, goal_state = clean_room(initial_location, initial_location_status, goal_state, cost)

    if initial_location_complement_status == 1:
        print(f"Moved to location {complement_location}.")
        cost += 1

        cost, goal_state = clean_room(complement_location, initial_location_complement_status, goal_state, cost)
    else:
        print(f"{complement_location} is already clean.")
